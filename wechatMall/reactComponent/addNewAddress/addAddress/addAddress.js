...
var addAddress = React.createClass({
  ...
  delet: function() {
    $.ajax({
      ...
      success: function(data) {
        if (data.resultcode == true) {
          window.location.href = "/wechatMall/authc/addressList.html";
        } else {
          alert(data.msg);
        }
      }.bind(this),
      ...
      success: function(data) {
        if (data.resultcode == true) {
          if (this.state.myOneYuanBuyState == 1) {
            window.location.href = "/wechatMall/authc/addressList.html?myOneYuanBuy=1&acId=" + this.state.acId + "&bhId=" + this.state.bhId + "&postageFree=" + this.state.postageFree;
          } else {
            window.location.href = "/wechatMall/authc/addressList.html";
          }
  ...
  render: function() {
            ...
            <textarea onChange={this.addressInfoChange} ref="myAddressInfo" value={this.state.addressInfoState} className={mmhc.street} name="addressInfo" id="" rows="3"></textarea>
    {
      /*<div className={mmhc.name}>邮编</div>
            ...
            <div onClick={this.noEmpty} className={cNs(mmhc.addAddressBottom,mmhc.submitButton)}>保存地址</div>
            {this.state.addressNum>1?<div onClick={this.delet} className={mmhc.delet}>删除</div>:""}
