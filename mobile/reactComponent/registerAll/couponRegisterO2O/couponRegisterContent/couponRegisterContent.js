...
var CouponRegisterContent = React.createClass({
  ...
  noEmpty: function() {
    ...
    $.ajax({
      ...
      url: "/wx/coop/doregisterAnthor.json",
      data: {
      ...
      success: function(data) {
        if (data == "注册成功") {
          alert("恭喜注册成功", function() {
            window.location.href = "/weixinSigningFee/authc/offlinePaymentO2O.html?userCode=" + that.state.userCode + "&shopCode=" + that.state.userCode + "&mobile=" + that.state.phoneState;
  ...
  nextFun: function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.success == 4) {
          $.ajax({
            url: jsonPath.path + '/o2o/o2oRelationship.json',
            ...
            success: function(data) {
              $.ajax({
                url: '/wx/coop/updateWeiXinOpenIdTwo.json',
                type: "post",
                error: function() {
                  alert("请求数据失败");
                },
                data: {
                  ...
                  smsVerifyCode: this.state.verificationState,
                  userCode: this.state.userCode,
                  sign: that.state.sign,
                  openId: openId,
                  userType: that.state.userType
                },
                success: function(data) {
                  if (data.flag == "success") {
                    alert("恭喜，账号[" + that.state.phoneState + "]成功绑定微信！", function() {
                      window.location.href = "/weixinSigningFee/authc/offlinePaymentO2O.html?userCode=" + that.state.userCode + "&shopCode=" + that.state.userCode + "&mobile=" + that.state.phoneState;
                    });
                  } else {
                    alert(data.message)
