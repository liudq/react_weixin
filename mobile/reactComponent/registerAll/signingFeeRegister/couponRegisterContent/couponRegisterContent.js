  ...
  noEmpty: function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data == "注册成功") {
          alert("注册成功", function() {
            window.location.href = "/signingFeeUser.html?userCode=" + that.state.userCode + "&mobile=" + that.state.phoneState + "&openId=" + openId;
          })
        } else {
          alert(data);
        };
      ...
      success: function(data) {
        if (data.success != 0) {
          if (data.success == 1) {
            ...
            alert(<div>恭喜您已经成为我们平台的会员若需升级，点击<a href='/weixinSigningFee/authc/signingFeeUserUpgrade.html'>升级</a>按钮升级</div>);
            // alert(data.message);
          } else if (data.success == 3) {
            window.location.href = "/weixinSigningFee/authc/signingFeeUserUpgrade.html";
          } else {
            this.setState({
              isNewState: 1
            }, function() {
              window.location.href = "/signingFeeUser.html?userCode=" + this.state.userCode + "&mobile=" + this.state.phoneState + "&openId=" + openId;
            })
          }
        } else {
          alert(data.message);
