...
var CouponRegisterContent = React.createClass({
  ...
  noEmpty: function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.success) {
          alert("注册成功", function() {
            window.location.href = "/cooperativeShops/firstIndex.html?userCode=" + data.userCode + "&newUserCode=" + data.newUserCode + "&sign=" + data.sign;
          })
        } else {
          alert(data.message)
        }
        // if (data == "注册成功") {
        //   alert("注册成功", function() {
        //     window.location.href = "/cooperativeShops/firstIndex.html?userCode=" + that.state.userCode;
        //     // if (that.state.tag * 1 == 1) {
        //     //   window.location.href = "/weixinSigningFee/authc/offlinePayment.html?userCode=" + that.state.userCode;
        //     // } else {
        //     //   window.location.href = "/weixin/couponRegisterSuccess.html?userCode=" + that.state.userCode;
        //     // }
  ...
  nextFun: function() {
    ...
    var that = this;
    $.ajax({
      url: "/wx/coop/mobileIsExist.json",
      type: "post",
      data: {
        mobile: this.state.phoneState,
        smsVerifyCode: this.state.verificationState,
        ...
        if (data.success != 0) {
          if (data.success == 1) {
            ...
            this.setState({
              ...
              $.ajax({
                url: '/getO2oKShopInformation.json',
                type: "post",
                data: {
                ...
                success: function(data) {
                  if (data.sr.success) {
                    if (data.sr.result.status != 7) {
                      window.location.href = "/cooperativeShops/firstIndex.html?userCode=" + data.userCode + "&newUserCode=" + data.newUserCode + "&sign=" + data.sign;
                    } else {
                      alert("您已经审核通过！");
                    }
                  } else {
                    alert(data.sr.message)
                  }
                }.bind(this),
                dataType: "json"
              });
              // alert("恭喜注册成功！", function() {
              //   window.location.href = "/firstIndex?userCode=" + that.state.userCode;
              // });
