  ...
  render: function() {
    return (
      <div className={cNs(this.props.commodity2,mmhc.background)}>
          <div className={mmhc.mmhSwiper} style={{"opacity":"0.99"}}>
          ...
          <div className={cNs(mmhc.spe,this.props.guiGeState)}>
            ...
            {this.props.toShareFlag==false?<div onClick={this.onClickShopping3} className={mmhc.speSce4Shopping}>立即购买</div>:<div onClick={this.toShare} className={mmhc.con5Right2}>输入兑换码</div>}
...
var SingleCommodity = React.createClass({
  ...
  onClickShopping2: function(speNum, goodsIdAll) {
    $.ajax({
      url: jsonPath.path + '/authc/seckill/Exchange.json',
  ...
  toSharehide: function() {
    this.setState({
      unlockPanelFlag: false
    });
  },
  unlockBtn: function() {
    var code = ReactDom.findDOMNode(this.refs.unlockCode).value;
    if (null == code || code == "") {
      alert("请输入您的兑换码");
    } else {
      $.ajax({
      ...
      <div>
        <div className={mmhc.titleAll}>
          <div className={mmhc.title}>
            {this.state.apkState==1?<a href="back" className={mmhc.titleLeft}><img className={mmhc.backImg} src={backImg} alt=""/></a>:<a href={"/exchange/tolistNew.html?activityId="+this.state.activityIdState} className={mmhc.titleLeft}><img className={mmhc.backImg} src={backImg} alt=""/></a>}
        ...
        <div className={mmhc.content5}>
          ...
          {
            this.state.toShareFlag==false
            ?<div onClick={this.onClickShopping} className={mmhc.con5Right2}>立即购买</div>
            :<div onClick={this.toShare} className={mmhc.con5Right2}>输入兑换码</div>
          }
          <div onClick={this.toSharehide} className={cNs(mmhc.unlockMask,this.state.unlockPanelFlag==true?mmhc.show:"")}></div>
          <div className={cNs(mmhc.unlockPanel,this.state.unlockPanelFlag==true?mmhc.show:"")}>
            <h1>请输入兑换码</h1>
            <input type="text" ref="unlockCode" />
            <a href="javascript:void(0)" onClick={this.unlockBtn}>确定</a>
          </div>
          <div className={mmhc.clear}></div>
        </div>
  ...
  componentWillMount: function() {
    ...
    $.ajax({
      url: jsonPath.path + '/seckill/checkRedeem.json',
      type: jsonPath.method,
      dataType: 'json',
      data: {
