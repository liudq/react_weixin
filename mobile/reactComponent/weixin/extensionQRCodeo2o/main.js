  ...
  render: function() {
    return (
      <div className={mmhc.content}>
        <MmhAlert/>
        <img style={{color:"#fff",display:this.state.displayFlag==false?"block":"none"}} onError={this.imgError} className={mmhc.qRCodeImg} src={this.state.imgUrl} alt="温馨提示：您不是线下店铺店主或者店员，不能生成收费二维码
。"/>
        <p style={{color:"#fff",display:this.state.displayFlag==true?"block":"none"}}>温馨提示：您不是线下店铺店主或者店员，不能生成收费二维码
。</p>
      </div>
    )
  },
