...
var OfflinePayment = React.createClass({
  ...
  submit: function() {
    ...
    if (that.state.noPayState == 0) {
      that.setState({
        ...
        $.ajax({
          url: jsonPath.path + '/authc/weixin/o2oPay.json',
  ...
  render: function() {
    return (
      <div className={mmhc.content}>
        <div className={mmhc.section1}>线下店铺：<i>{this.state.shopName}</i></div>
        <input onChange={this.inputMoney} placeholder="请输入消费金额,单位元" className={mmhc.payInput} type="text"/>
        <div onClick={this.submit} className={mmhc.button}>提交</div>
      </div>
    )
  },
  componentDidMount: function() {
    ...
    //获取店铺的名字
    $.ajax({
      url: jsonPath.path + '/authc/weixin/findShopName.json',
      type: jsonPath.method,
      error: function() {
        alert("请求数据失败");
      },
