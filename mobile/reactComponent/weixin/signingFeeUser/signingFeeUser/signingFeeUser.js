...
var SigningFeeUser = React.createClass({
  ...
  nextButton: function() {
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.rst == 1) {
          window.location.href = "/weixinSigningFee/authc/signingFeePay.html?openId=" + this.state.openId + "&userCode=" + this.state.userCode;
        } else {
          alert(data.msg)
        }
      }.bind(this),
  ...
  render: function() {
    return (
      <div className={mmhc.pay}>
        <form id="postForm" action="/wx/updateCer.json" encType="multipart/form-data"  method="post">
