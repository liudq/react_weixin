    ...
    return (
      <div className={mmhc.adressListContent}>
        <div className={cNs(mmhc.title,this.state.versionTitel==1?mmhc.title12:"")}>
          ...
          <div className={mmhc.titleCenter}>收货地址</div>
        </div>
        ...
        {
          this.state.myOneYuanBuyState==1?<a href={"/authc/addAddress.html?num=-1&myOneYuanBuy=1&acId="+this.state.acId+"&bhId="+this.state.bhId+"&postageFree="+this.state.postageFree} className={mmhc.newAddress}>+新建地址</a>:<a href="/authc/addAddress.html?num=-1" className={mmhc.newAddress}>+新建地址</a>
        }
        </div>
      </div>
    )
