    ...
    $.ajax({
      ...
      success: function(data) {
        if (this.state.myOneYuanBuy == 1) {
          window.location.href = "/authc/seckillpay/info.html?addressId=" + data.address.id + "&acId=" + this.state.acId + "&bhId=" + this.state.bhId + "&postageFree=" + this.state.postageFree;
        } else {
          window.location.href = "/authc/order.html?addressId=" + data.address.id;
        }
      }.bind(this),
      dataType: "json"
    });
    ...
    return (
      <div className={mmhc.addressContentHeight}>
        ...
        <div className={mmhc.addressRight}>
          <a href={"/authc/addAddress.html?id="+this.props.addressList.id+"&num="+this.props.num}>更改</a>
          <div className={mmhc.clear}></div>
        </div>
        <div className={mmhc.clear}></div>
      </div>
