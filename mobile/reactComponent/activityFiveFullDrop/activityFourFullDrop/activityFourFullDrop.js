    ...
    return (
      <div className={mmhc.content}>
          <div className={cNs(mmhc.title,this.state.apkState==1?mmhc.show:mmhc.hide)}>
            ...
            <a className={mmhc.shareBaobao} href={"sharebaobao,"+eshopHomePath+"/activityFiveFullDropShare.html,母婴夏末初秋,换新季"}>
              ...
              return  <div key={index} id={index} className={cNs(mmhc.section3,index==0?mmhc.block:"")}>
                        {
                           activityProductVOList.productList.map(function(productList, index) {
                            return  <a key={index} href={this.state.apkState==1?"/detail.html?prdCode="+productList.id+"&activityId="+this.state.activityId+"&tabNum="+this.state.tabNum+"&returnUrl=/activityFiveFullDrop.html":"/wechatMall/detail.html?prdCode="+productList.id+"&activityId="+this.state.activityId+"&tabNum="+this.state.tabNum+"&returnUrl=/activityFiveFullDrop.html"} key={index} className={mmhc.singleGoods}>
