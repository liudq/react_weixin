...
var mmhc = require("./activityChildFour.css");
...
var ActivityChildFour = React.createClass({
    ...
    return (
      <div className={mmhc.content}>
          <div className={cNs(mmhc.title,this.state.versionTitel==1?mmhc.titel12:"")}>
            ...
            童装专场 1折起
            <a className={mmhc.shareBaobao} href={"sharebaobao,"+eshopHomePath+"/activity/activityChildFourShare.html,童装一折起,时尚萌宝的夏日时装秀！"}>
          ...
          <div className={mmhc.bannerContainer}>
            ...
            <div className={cNs(mmhc.rule,this.state.ruleShowFlag==true?mmhc.show:"")}>
              <h1>活动规则</h1>
              <p>1、不限购买数量，不限购买次数；</p>
              <p>2、端午节童装1折起活动专场享79元包邮；</p>
              <p>3、活动时间：6月9日-6月15日。</p>
              ...
              return  <div key={index} id={index} className={cNs(mmhc.section3,index==0?mmhc.block:"")}>
                        {
                           activityProductVOList.productList.map(function(productList, index) {
                            return  <a key={index} href={"/detail.html?prdCode="+productList.id} key={index} className={mmhc.singleGoods}>
                                      <div className={mmhc.picCon}>
                                        <img src={imgPath+productList.masterImg} alt=""/>
                                        <i>{productList.discount?productList.discount:3}折</i>
                                      </div>
  ...
  componentDidMount: function() {
    ...
    $.ajax({
      //url: "/json/activity/activityProductById.json?activityId=" + this.state.activityId,
