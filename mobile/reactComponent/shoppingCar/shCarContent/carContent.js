  ...
  render: function() {
    return (
      <div className={mmhc.div1}>
        ...
        <a className={mmhc.shCarA} href="homePage">去逛逛</a>
      </div>
    )
  },
...
var ShCarlist = React.createClass({
  ...
  jieSuanButtonFun: function() {
    if (this.state.allChoiceId == 0) {
      ...
      window.location.href = "/authc/order.html?cartIds=" + this.state.allChoiceId;
    }
  },
  render: function() {
    return (
      <div>
        <div className={cNs(mmhc.yShTiele,this.props.versionTitel==1?mmhc.titel12:"")}>{this.state.fromHome==1?"":<a className={mmhc.backCss} href="javascript:history.go(-1);"><img src={this.props.versionTitel==1?back2Img:backImg} alt=""/></a>}购物车({this.state.totalNumber})</div>
        <div className={mmhc.yShTiele2}>11</div>
