    ...
    return (
      <div className={mmhc.content}>
          <div className={cNs(mmhc.title,this.state.apkState==1?mmhc.show:mmhc.hide)}>
            ...
            <a className={mmhc.shareBaobao} href={"sharebaobao,"+eshopHomePath+"/activityFourFullDropShare.html,fashionShow,萌宝金秋时尚"}>
              ...
              return  <div key={index} id={index} className={cNs(mmhc.section3,index==0?mmhc.block:"")}>
                        {
                           activityProductVOList.productList.map(function(productList, index) {
                            return  <a key={index} href={this.state.apkState==1?"/detail.html?prdCode="+productList.id+"&activityId="+this.state.activityId+"&tabNum="+this.state.tabNum+"&returnUrl=/activityFourFullDrop.html":"/wechatMall/detail.html?prdCode="+productList.id+"&activityId="+this.state.activityId+"&tabNum="+this.state.tabNum+"&returnUrl=/activityFourFullDrop.html"} key={index} className={mmhc.singleGoods}>
                                      <div className={mmhc.picCon}>
                                        <img src={imgPath+productList.masterImg} alt=""/>
                                        {/*<i>{productList.discount?productList.discount:0}折</i>*/}
                                      </div>
                                      <p className={mmhc.sec3Title}>{productList.name1.length>21?productList.name1.slice(0,21)+"...":productList.name1}</p>
